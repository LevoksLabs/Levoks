...
test(
  "durable vault encrypts, isolates identities/projects, detects tampering and rotates keys without exposing values",
  { timeout: 180000 },
  async () => {
    ...
    try {
      ...
      assert.equal(
        await vault.resolve("alice", "project_a", "API_KEY"),
        secret,
      );
      assert.deepEqual(await vault.list("bob", "project_a"), []);
      assert.deepEqual(await vault.list("alice", "project_b"), []);
      await assert.rejects(
        vault.resolve("bob", "project_a", "API_KEY"),
        /does not exist/,
      );
      await assert.rejects(
        vault.put("alice", "project_a", "API_KEY", "overwrite", 0),
        /changed/,
      );
      const rotated = new MongoVault(db, {
        active: "v2",
      ...
      assert.equal(
        await new MongoVault(db, {
          active: "v2",
          keys: { v2: newKey },
        }).resolve("alice", "project_a", "API_KEY"),
        secret,
      );
      assert.doesNotMatch(
        JSON.stringify(await rotated.list("alice", "project_a")),
        /encrypted|nonce|tag|test-provider/,
      );
      await db
        .collection("levoks_secrets")
        .updateOne(
          { name: "API_KEY" },
          { $set: { tag: randomBytes(16).toString("base64") } },
        );
      await assert.rejects(
        rotated.resolve("alice", "project_a", "API_KEY"),
        /could not be decrypted/,
      );
      await assert.rejects(
        rotated.remove("alice", "project_a", "API_KEY", 1),
        /changed/,
      );
